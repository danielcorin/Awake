...
build_setting() {
    ...
    awk -F ' = ' -v wanted="$wanted" -v target="$SCHEME" '
        /^Build settings for action .* and target / {
            in_target = index($0, "target " target ":") > 0
            next
        }
        !in_target { next }
        {
...
verify_app() {
    ...
    "$SCRIPT_DIR/verify-no-coverage.sh" "$APP_PATH"
...
if [[ -z "$PROJECT" ]]; then
    ...
    [[ -n "$PROJECT" ]] || fail "no .xcodeproj found in $REPO_ROOT; run 'mise exec -- xcodegen generate' or pass --project"
fi
...
if [[ "$BUILD_FROM_SOURCE" -eq 1 ]]; then
    ...
    "$SCRIPT_DIR/verify-no-coverage.sh" "$ARCHIVE_PATH/Products/Applications/$PRODUCT_NAME.app"
